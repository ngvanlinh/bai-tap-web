                    ...
                    <div class="<col-lg-12 col-md-12 col-xs-12 col-sm-12>">
                        <?php 
                            ...
                            if($_SERVER['REQUEST_METHOD']=='POST'){
                                ...
                                if(empty($errors)){
                                $query="update khachhang set tenkhachhang='{$tenkhachhang}', diachi='{$diachi}' ,sdt='{$sdt}', email='{$email}' where id={$id}";
                        ...
                        //đổ dữ liệu vào form sửa tt khách hàng
                        $query_id="select tenkhachhang,diachi,sdt,email from khachhang where id={$id}";
                        $result_id=mysqli_query($conn,$query_id) or die("Quyery {$query_id} \n <br> Mysql errors:" .mysqli_error($conn));
                        //kiểm tra id có tồn tại hay ko
                        if (mysqli_num_rows($result_id)==1) {
                            list($tenkhachhang,$diachi,$sdt,$email)=mysqli_fetch_array($result_id,MYSQLI_NUM);
                        }else{
                            $message="<p class='required'>khách hàng không tồn tại</p>";
                        }
                        ?>
                        <form name="add_khachhang" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="tenkhachhang" value="<?php if(isset($tenkhachhang)){ echo $tenkhachhang;}?>" class="form-control" placeholder="Tên khách hàng">
                            </div>
                            <div class="form-group">
                                <label>Địa chỉ</label>
                                <input type="text" value="<?php if(isset($diachi)){ echo $diachi;}?>" name="diachi" class="form-control" placeholder="Địa chỉ">
                            </div>
                            <div class="form-group">
                            ...
                            <input name="submit" type="submit" class="btn btn-primary" value="Cập nhập">
                        </form>
                    </div>
                </div>
                <!-- /.row -->
